...
if (isset($_GET["delete"])) {
    ...
    $checked = htmlspecialchars($_POST['deletesubscription']);
...
while ($arr = mysqli_fetch_assoc($res)) {
    ...
    $postid   = intval($arr2["id"]);
    $posterid = intval($arr2["userid"]);
    $queryuser = sql_query("SELECT username FROM users WHERE id=" . sqlesc($arr2['userid'])) or sqlerr(__FILE__, __LINE__);
