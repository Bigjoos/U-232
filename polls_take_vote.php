...
if (!$_POST['nullvote']) {
    ...
    sql_query("INSERT INTO poll_voters (user_id, ip_address, poll_id, vote_date)
                        VALUES ({$CURUSER['id']}," . sqlesc($CURUSER['ip']) . ",{$poll_data['pid']}," . time() . ")") or sqlerr(__FILE__, __LINE__);
    ...
    sql_query("INSERT INTO poll_voters (user_id, ip_address, poll_id, vote_date)
                VALUES
                ({$CURUSER['id']}, " . sqlesc($CURUSER['ip']) . ", {$poll_data['pid']}," . time() . ")") or sqlerr(__FILE__, __LINE__);
