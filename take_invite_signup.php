...
$a = (mysqli_fetch_row(sql_query('SELECT COUNT(*) FROM users WHERE email = ' . sqlesc($email)))) or sqlerr(__FILE__, __LINE__);
if ($a[0] != 0)
    stderr('Error', 'The e-mail address <b>' . htmlspecialchars($email) . '</b> is already in use.');

//=== check if ip addy is already in use
$c = (mysqli_fetch_row(sql_query("select count(*) from users where ip='" . sqlesc($_SERVER['REMOTE_ADDR']) . "'"))) or sqlerr(__FILE__, __LINE__);
if ($c[0] != 0)
...
$new_user       = sql_query("INSERT INTO users (username, passhash, secret, passhint, hintanswer, editsecret, invitedby, email, " . (!$arr[0] ? "class, " : "") . "added, last_access, last_login, time_offset, dst_in_use) VALUES (" . implode(",", array_map("sqlesc", array(
    $wantusername,
    $wantpasshash,
    $secret,
    $editsecret,
