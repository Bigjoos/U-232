...
function peer_list($array)
{
    ...
    foreach ($array as $p) {
        ...
        $htmlout .= "<tr>
       		<td align='center'><a href='userdetails.php?id=" . intval($p["p_uid"]) . "' >" . htmlspecialchars($p["p_user"]) . "</a></td>
...
if (strlen($search) > 4) {
    ...
    $where = "WHERE t.name LIKE '" . sqlesc($letter) . "%'";
    $p     = "letter=" . sqlesc($letter) . "&amp;";
} else {
...
$count = mysqli_fetch_row(sql_query("SELECT count(*) from torrents as t $where")) or sqlerr(__FILE__, __LINE__);
$perpage = 10;
        ...
        $htmlout .= "<tr>
		 <td align='center' valign='top' nowrap='nowrap'>
        	<table align='center' width='160' border='1' cellpadding='2'>
                ...
                <tr><td align='center'>" . ($row["poster"] ? "<a href=\"" . htmlspecialchars($row["poster"]) . "\"><img src=\"" . htmlspecialchars($row["poster"]) . "\" border=\"0\" width=\"150\" height=\"195\" alt=\"No Poster\" title=\"No Poster\" /></a>" : "<img src=\"pic/noposter.png\" border=\"0\" width=\"150\" alt=\"No Poster\" title=\"No Poster\" />") . "</td></tr>
            </table>
			...
			<tr>
				<td align='center' width='100%' rowspan='2' ><a href='details.php?id=" . intval($row["id"]) . "&amp;hit=1'><b>" . substr($row["name"], 0, 60) . "</b></a></td>
			...
			<tr>
				...
				<td align='center' nowrap='nowrap'>" . ($row["snatched"] > 0 ? ($row["snatched"] == 1 ? intval($row["snatched"]) . " time" : intval($row["snatched"]) . " times") : 0) . "</td>
				<td align='center'>" . intval($row["seeders"]) . "</td>
				<td align='center'>" . intval($row["leechers"]) . "</td>
			</tr>
			<tr><td width='100%' colspan='6' class='colhead' >Info.</td></tr>
			<tr><td width='100%' colspan='6' style='font-weight:normal;' >" . readMore($row["descr"], 500, "details.php?id=" . intval($row["id"]) . "&amp;hit=1") . "</td></tr>
