...
function _torrenttable($res, $frame_caption)
{
    ...
    while ($a = mysqli_fetch_assoc($res)) {
        ...
        if ($a["leechers"]) {
            ...
            $ratio = $lang['common_infratio'];
        $htmlout .= "<tr><td align='center'>$num</td><td align='left'><a href='details.php?id=" . $a["id"] . "&hit=1'><b>" . $a["name"] . "</b></a></td><td align='right'>" . number_format($a["times_completed"]) . "</td><td align='right'>" . mksize($a["data"]) . "</td><td align='right'>" . number_format($a["seeders"]) . "</td><td align='right'>" . number_format($a["leechers"]) . "</td><td align='right'>" . ($a["leechers"] + $a["seeders"]) . "</td><td align='right'>$ratio</td>\n";
    }
    $htmlout .= end_table();
    $htmlout .= end_frame();
