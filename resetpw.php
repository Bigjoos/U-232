...
elseif ($step == '2') {
    ...
    if ($fetch['hintanswer'] != md5($answer)) {
        ...
        sql_query('INSERT INTO messages (receiver, msg, subject, added) VALUES (' . sqlesc((int) $fetch['id']) . ', ' . sqlesc($msg) . ', ' . sqlesc($subject) . ', ' . sqlesc(time()) . ')') or sqlerr(__FILE__, __LINE__);
...
<input type='hidden' name='id' value='" . (int) $fetch['id'] . "' /></td></tr></table></form>";
    ...
    sql_query('UPDATE users SET secret = ' . sqlesc($secret) . ', editsecret = "", passhash=' . sqlesc($newpassword) . ' WHERE id = ' . sqlesc($id) . ' AND editsecret = ' . sqlesc($fetch["editsecret"])) or sqlerr(__FILE__, __LINE__);
