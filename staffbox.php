...
switch ($do) {
    ...
    case 'setanswered':
        if ($id > 0) {
            ...
            sql_query('INSERT INTO messages(sender,receiver,added,subject,msg) VALUES(' . sqlesc($CURUSER['id']) . ',' . sqlesc($a['sender']) . ',' . time() . ',' . sqlesc('RE: ' . $a['subject']) . ',' . sqlesc($response) . ')') or sqlerr(__FILE__, __LINE__);
            ...
            if (mysqli_num_rows($q2) == 1) {
                ...
                $HTMLOUT .= "<form action='" . $_SERVER['PHP_SELF'] . "' method='post'>
								<table width='80%' border='1' cellspacing='0' cellpadding='5' align='center'>
								 <tr><td>{$lang['staffbox_pm_from']}&nbsp;<a href='userdetails.php?id=" . intval($a['sender']) . "'>" . htmlspecialchars($a['username']) . "</a> at " . get_date($a['added'], 'DATE', 1) . "<br/>
								 {$lang['staffbox_pm_subject']} : <b>" . htmlspecialchars($a['subject']) . "</b><br/>
								 {$lang['staffbox_pm_answered']} : <b>" . ($a['answeredby'] > 0 ? "<a href='userdetails.php?id=" . intval($a['answeredby']) . "'>" . htmlspecialchars($a['username2']) . "</a>" : "<span style='color:#ff0000'>No</span>") . "</b>
								</td></tr>
								...
								<tr><td align='left'>
									...
									<input type='hidden' name='id[]' value='" . intval($a['id']) . "'/><input type='submit' value='{$lang['staffbox_confirm']}' />
									</td></tr>
								</table>
								</form>";
    ...
    default:
        ...
        if (!$count_msgs)
            ...
            while ($a = mysqli_fetch_assoc($r))
                $HTMLOUT .= "<tr>
                   ...
                   <td align='center'><b>" . ($a['username'] ? "<a href='userdetails.php?id=" . $a['sender'] . "'>" . htmlspecialchars($a['username']) . "</a>" : "Unknown[" . intval($a['sender']) . "]") . "</b></td>
                   <td align='center' nowrap='nowrap'>" . get_date($a['added'], 'DATE', 1) . "<br/><span class='small'>" . get_date($a['added'], 0, 1) . "</span></td>
				   <td align='center'><b>" . ($a['answeredby'] > 0 ? "by <a href='userdetails.php?id=" . intval($a['answeredby']) . "'>" . htmlspecialchars($a['username2']) . "</a>" : "<span style='color:#ff0000'>No</span>") . "</b></td>
                   <td align='center'><input type='checkbox' name='id[]' value='" . intval($a['id']) . "' /></td>
                  </tr>\n";
        ...
        echo stdhead($lang['staffbox_head']) . $HTMLOUT . stdfoot($stdfoot);
}
