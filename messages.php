...
if (!$action)
{
    ...
    $pmcount = mysqli_fetch_row(sql_query("SELECT COUNT(*) FROM messages WHERE receiver=".sqlesc($CURUSER['id'])." AND location >= '1' || sender=".sqlesc($CURUSER['id'])." AND saved = 'yes' ")) or sqlerr(__FILE__,__LINE__);
    ...
    else
    {
      while ($row = mysqli_fetch_assoc($res))
      {
        ...
        {
          $res2 = sql_query("SELECT username FROM users WHERE id=" . sqlesc($row['sender'])) or sqlerr(__FILE__,__LINE__);
          $username = mysqli_fetch_array($res2);
          $username = "<a href='userdetails.php?id=".intval($row['sender'])."'>".htmlspecialchars($username[0])."</a>";
        ...
        <a href='messages.php?action=viewmessage&amp;id=".intval($row['id'])."'>$subject</a></td>
        <td align='left'>$username</td>
        <td nowrap='nowrap'>" . get_date($row['added'], '') . "</td>
        <td><input type='checkbox' name='messages[]' value='".intval($row['id'])."' /></td>\n</tr>";
        }
    }
    ...
    if ($action == "viewmessage")
    {
      ...
      if ($message['sender'] == $CURUSER['id'])
      {
        ...
        $sender = "<a href=\"userdetails.php?id=" . intval($message['receiver']) . "\">" . htmlspecialchars($sender[0]) . "</a>";
        $reply = "";
        $from = "To";
      }
      else
      {
        ...
        else
        {
          ...
          $sender = mysqli_fetch_array($res2);
          $sender = "<a href='userdetails.php?id=".intval($message['sender'])."'>".htmlspecialchars($sender[0])."</a>";
          $reply = "<a href='sendmessage.php?receiver=".intval($message['sender'])."&amp;replyto={$pm_id}'><span class='btn'>{$lang['messages_reply']}</span></a>";
        }
      }
      ...
      while ($row = mysqli_fetch_assoc($res))
      {
        $HTMLOUT .= "<option value='".intval($row['boxnumber'])."'>" . htmlspecialchars($row['name']) . "</option>\n";
      }
    ...
    if ($action == "moveordel")
    {
      ...
      $pm_messages = htmlspecialchars($_POST['messages']);
      ...
      if (isset($_POST['move']))
      {
        if ($pm_id)
        {
          ...
          sql_query("UPDATE messages SET location=" . sqlesc($pm_box) . " WHERE id=" . sqlesc($pm_id) . " AND receiver=" . sqlesc($CURUSER['id']) . " LIMIT 1") or sqlerr(__FILE__,__LINE__);
        }
        else
        {
          // Move multiple messages
          sql_query("UPDATE messages SET location=" . sqlesc($pm_box) . " WHERE id IN (" . implode(", ", array_map("sqlesc",$pm_messages)) . ') AND receiver=' .sqlesc($CURUSER['id'])) or sqlerr(__FILE__,__LINE__);
        }
      ...
      elseif ($_POST['delete'])
      {
        ...
        else
        {
          ...
          {
            $res = sql_query("select * FROM messages WHERE id=" . sqlesc((int)$id)) or sqlerr(__FILE__,__LINE__);
            $message = mysqli_fetch_assoc($res);
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
      ...
      $from = intval($message['sender']);
      $orig = intval($message['receiver']);
      ...
      else
      {
        ...
        $res = sql_query("SELECT id FROM users WHERE LOWER(username)=LOWER(" . sqlesc($username) . ") LIMIT 1") or sqlerr(__FILE__,__LINE__);
        if (!$res)
        {
        ...
        else
        {
          ...
          $from = htmlspecialchars($from['username']);
        }
        ...
        if ($CURUSER['class'] < UC_MODERATOR)
        {
          ...
          {
            $res2 = sql_query("SELECT * FROM blocks WHERE userid=".sqlesc($to)." AND blockid=" . sqlesc($CURUSER["id"])) or sqlerr(__FILE__, __LINE__);
            if (mysqli_num_rows($res2) == 1)
            stderr("{$lang['messages_refused']}", "{$lang['messages_blocked']}");
          }
          elseif ($from["acceptpms"] == "friends")
          {
            $res2 = sql_query("SELECT * FROM friends WHERE userid=".sqlesc($to)." AND friendid=" . sqlesc($CURUSER["id"])) or sqlerr(__FILE__, __LINE__);
            if (mysqli_num_rows($res2) != 1)
        ...
        sql_query("INSERT INTO messages (poster, sender, receiver, added, subject, msg, location, saved) VALUES(".sqlesc($CURUSER["id"]).", ".sqlesc($CURUSER["id"]).", ".sqlesc($to).", " . sqlesc(time()) . ", " . sqlesc($subject) . "," .
    ...
    if ($action == "editmailboxes2")
    {
      ...
      if ($action2 == "add")
      {
        ...
        $res = sql_query("select MAX(boxnumber) FROM pmboxes WHERE userid=" . sqlesc($CURUSER['id'])) or sqlerr(__FILE__,__LINE__);
        ...
        {
          ++$box;
          sql_query("INSERT INTO pmboxes (userid, name, boxnumber) VALUES (" . sqlesc($CURUSER['id']) . ", " . sqlesc($name1) . ", ".sqlesc($box)."") or sqlerr(__FILE__,__LINE__);
        }
        
        if (strlen($name2) > 0)
        {
          ++$box;
          sql_query("INSERT INTO pmboxes (userid, name, boxnumber) VALUES (" . sqlesc($CURUSER['id']) . ", " . sqlesc($name2) . ", ".sqlesc($box)."") or sqlerr(__FILE__,__LINE__);
        }
        
        if (strlen($name3) > 0)
        {
          ++$box;
          sql_query("INSERT INTO pmboxes (userid, name, boxnumber) VALUES (" . sqlesc($CURUSER['id']) . ", " . sqlesc($name3) . ", ".sqlesc($box)."") or sqlerr(__FILE__,__LINE__);
        }
            ...
            if (isset($_GET['edit' . $row['id']]))
            {
              if ($_GET['edit' . $row['id']] != $row['name'])
              {
                ...
                else
                {
                  ...
                  sql_query("UPDATE messages SET location=0 WHERE saved='yes' AND location=" . sqlesc($row['boxnumber']) . " AND receiver=" . sqlesc($CURUSER['id'])) or sqlerr(__FILE__,__LINE__);
                  sql_query("UPDATE messages SET saved='no' WHERE saved='yes' AND sender=" . sqlesc($CURUSER['id'])) or sqlerr(__FILE__,__LINE__);
                  sql_query("DELETE FROM messages WHERE saved='no' AND location=" . sqlesc($row['boxnumber']) . " AND receiver=" . sqlesc($CURUSER['id'])) or sqlerr(__FILE__,__LINE__);
                  sql_query("DELETE FROM messages WHERE location=0 AND saved='yes' AND sender=" . sqlesc($CURUSER['id'])) or sqlerr(__FILE__,__LINE__);
                }
              }
            }
          }
