...
if (!$action || $action == "show") {
    ...
    if ($count == 0) {
        ...
        while ($arr = mysqli_fetch_assoc($res)) {
            ...
            <td>{$ratio}</td>
            <td>{$status}</td>
            <td><input type=\"checkbox\" name=\"deleteapp[]\" value=\"" . intval($arr['id']) . "\" /></td>
            </tr>\n";
        }
        $HTMLOUT .= "</table>
...
if ($action == "acceptapp") {
    ...
    $res = sql_query("SELECT uploadapp.id, users.username, users.modcomment, users.id AS uid FROM uploadapp INNER JOIN users on uploadapp.userid = users.id WHERE uploadapp.id = " . sqlesc($id)) or sqlerr(__FILE__, __LINE__);
    $arr        = mysqli_fetch_assoc($res);
    ...
    sql_query("UPDATE users SET class = 3, modcomment = " . sqlesc($modcomment) . " WHERE id=" . sqlesc($arr['uid']) . " AND class < " . UC_MODERATOR . "") or sqlerr(__FILE__, __LINE__);
