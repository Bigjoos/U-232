...
$unconnectables = intval($row['unconnectables']);
...
if (file_exists($news_file) && filemtime($news_file) > (time() - $expire)) {
    ...
    while ($news1 = mysqli_fetch_assoc($res)) {
        $news2[] = $news1;
    }
...
if ($news2) {
    foreach ($news2 as $array) {
        ...
        if ($news_flag < 2) {
            $HTMLOUT .= "<a href=\"javascript: klappe_news('a" . intval($array['id']) . "')\"><img border=\"0\" src='pic/plus.gif' id=\"pica" . intval($array['id']) . "\" alt=\"Show/Hide\" />" . " - " . get_date($array['added'], 'DATE') . " - " . "" . htmlspecialchars($array['title']) . "</a></span>{$button}</div>";
            $HTMLOUT .= "<div id=\"ka" . intval($array['id']) . "\" style=\"display:" . ($array["sticky"] == "yes" ? "" : "none") . ";margin-left:30px;margin-top:10px;\"> " . format_comment($array["body"], 0) . " </div><br /> ";
...
if (mysqli_num_rows($topicres) > 0) {
    ...
    while ($topicarr = mysqli_fetch_assoc($topicres)) {
