...
if (isset($_POST["form"]) != "1") {
    ...
    elseif ($arr['status'] == 'rejected')
        ...
        <td><input name='userid' type='hidden' value='" . intval($CURUSER['id']) . "' />" . htmlspecialchars($CURUSER['username']) . "</td>
        </tr>
        <tr>
    ...
    if (!$res) {
        ...
        while ($arr = mysqli_fetch_assoc($subres))
            sql_query("INSERT INTO messages(sender, receiver, added, msg, subject, poster) VALUES(0, " . sqlesc($arr['id']) . ", " . sqlesc($dt) . ", " . sqlesc($msg) . ", " . sqlesc($subject) . ", 0)") or sqlerr(__FILE__, __LINE__);
