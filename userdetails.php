...
function maketable($res)
{
    ...
    foreach ($res as $arr) {
        ...
        $htmlout .= "<tr><td style='padding: 0px'>$catimage</td>\n" . "<td><a href='details.php?id=" . intval($arr['torrent']) . "&amp;hit=1'><b>" . htmlspecialchars($arr["torrentname"]) . "</b></a></td><td align='center'>$size</td><td align='right'>$seeders</td><td align='right'>$leechers</td><td align='center'>$uploaded</td>\n" . "<td align='center'>$downloaded</td><td align='center'>$ratio</td></tr>\n";
    }
    $htmlout .= "</table>\n";
    return $htmlout;
}
...
$user = mysqli_fetch_assoc($r) or bark("{$lang['userdetails_no_user']}");
...
$lastseen = intval($user["last_access"]);
if ($lastseen == 0)
...
$res = sql_query("SELECT COUNT(*) FROM comments WHERE user=" . sqlesc($user['id'])) or sqlerr(__FILE__, __LINE__);
$arr3            = mysqli_fetch_row($res);
$torrentcomments = intval($arr3[0]);

$res = sql_query("SELECT COUNT(*) FROM posts WHERE userid=" . sqlesc($user['id'])) or sqlerr(__FILE__, __LINE__);
$arr3       = mysqli_fetch_row($res);
$forumposts = intval($arr3[0]);
...
$agent = $a[2];
if (!empty($port))
    $HTMLOUT .= "<tr><td class='rowhead'>{$lang['userdetails_port']}</td><td class='tablea' align='left'>" . intval($port) . "</td></tr>
...
$snatches = "";
//start of Exapanding
$r = sql_query("SELECT id, name, seeders, leechers, category FROM torrents WHERE owner=" . sqlesc($id) . " ORDER BY name") or sqlerr(__FILE__, __LINE__);
if (mysqli_num_rows($r) > 0)
...
//==Snatched
$snatches = '';
$res = sql_query("SELECT s.*, t.name AS name, c.name AS catname, c.image AS catimg FROM snatched AS s INNER JOIN torrents AS t ON s.torrentid = t.id LEFT JOIN categories AS c ON t.category = c.id WHERE s.userid = " . sqlesc($user['id'])) or sqlerr(__FILE__, __LINE__);
...
if ($CURUSER['class'] >= UC_STAFF) {
    ...
    <td align='center' class='$class'>Seeds: " . intval($arr['seeders']) . "<br />Leech: " . intval($arr['leechers']) . "</td><td align='center' class='$class'><font color='lightgreen'>Uploaded:<br />" . "
    ...
    </td><td align='center' class='$class'>" . $arr["agent"] . "<br />port: " . htmlspecialchars($arr["port"]) . "<br />" . ($arr["connectable"] == 'yes' ? "<b>Connectable:</b> <font color='lightgreen'>Yes</font>" . "
...
if (isset($showpmbutton))
    $HTMLOUT .= "<tr>
      ...
      <form method='get' action='sendmessage.php'>
        <input type='hidden' name='receiver' value='" . intval($user["id"]) . "' />
...
if ($CURUSER['class'] >= UC_STAFF && $user["class"] < $CURUSER['class']) {
    ...
    if ($CURUSER['class'] >= UC_STAFF) {
        ...
        if ($leechwarn) {
            if ($user['leechwarn'] == 1)
                ...
                $HTMLOUT .= "<td align='center'>Until " . get_date($user['leechwarn'], 'DATE') . " (" . mkprettytime($user['leechwarn'] - time()) . " to go)</td></tr>";
        } else {
    ...
    if ($CURUSER['class'] >= UC_STAFF) {
        ...
        if ($warned) {
            if ($user['warned'] == 1)
                ...
                $HTMLOUT .= "<td align='center'>Until " . get_date($user['warned'], 'DATE') . " (" . mkprettytime($user['warned'] - time()) . " to go)</td></tr>";
        } else {
    ...
    $HTMLOUT .= "<tr><td class='rowhead'>Forum Mod</td><td colspan='2' align='left'><input name=\"forum_mod\" value=\"yes\" type=\"radio\" " . ($user["forum_mod"] == "yes" ? " checked='checked'" : "") . " />Yes <input name=\"forum_mod\" value=\"no\" type=\"radio\" " . ($user["forum_mod"] == "no" ? " checked='checked'" : "") . " />No</td></tr>\n";
    if ($user["forum_mod"] == "yes") {
        $r = sql_query("SELECT id,name,description FROM forums WHERE place = -1 ORDER BY name ASC") or sqlerr(__FILE__, __LINE__);
        $forumsc = mysqli_num_rows($r);
    ...
    <input type='radio' name='forums_" . $i . "' value='yes_" . intval($a["id"]) . "' " . (stristr($user["forums_mod"], "[" . $a["id"] . "]") == true ? "checked='checked'" : "") . " title='Set moderator for this forum' />
    <input type='radio' name='forums_" . $i . "' value='no_" . intval($a["id"]) . "' " . (stristr($user["forums_mod"], "[" . $a["id"] . "]") != true ? "checked='checked'" : "") . " title='Unset moderator for this forum' /></td></tr>";
            $i++;
        }
        $HTMLOUT .= "</table>
		</div>
