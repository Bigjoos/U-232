...
$forum_pics = array(
    ...
    'arrow_up' => 'p_up.gif',
    'online_btn' => 'user_online.gif',
    'offline_btn' => 'user_offline.gif',
    'pm_btn' => 'pm.gif',
    'p_report_btn' => 'report.gif',
    'p_quote_btn' => 'p_quote.gif',
    'p_delete_btn' => 'p_delete.gif',
    'p_edit_btn' => 'p_edit.gif'
);
/**
 * Configs End
 */
...
function subforums($arr)
{
    ...
    $i   = 0;
    foreach ($arr as $k) {
        $sub .= "&nbsp;<img src=\"pic/bullet_" . ($k["new"] == 1 ? "green.png" : "white.png") . "\" width=\"8\" title=\"" . ($k["new"] == 1 ? "New posts" : "Not new post") . "\" border=\"0\" alt='Subforum' /><a href=\"forums.php?action=viewforum&amp;forumid=" . $k["id"] . "\">" . htmlspecialchars($k["name"]) . "</a>" . ((count($arr) - 1) == $i ? "" : ",");
        $i++;
    }
    $sub .= "</font>";
    return $sub;
...
function showMods($ars)
{
    ...
    $count = count($ars);
    foreach ($ars as $a) {
        $mods .= "<a href=\"userdetails.php?id=" . intval($a["id"]) . "\">" . htmlspecialchars($a["user"]) . "</a>" . (($count - 1) == $i ? "" : " ,");
        $i++;
    }
    $mods .= "</font>";
    return $mods;
...
function catch_up($id = 0)
{
    ...
    while ($arr = mysqli_fetch_assoc($res)) {
        ...
        if (!is_valid_id($arr['r_id']))
            sql_query("INSERT INTO readposts (userid, topicid, lastpostread) VALUES(" . sqlesc($userid) . ", " . sqlesc($arr['id']) . ", " . sqlesc($postid) . ")") or sqlerr(__FILE__, __LINE__);
        else if ($arr['lastpostread'] < $postid)
            sql_query("UPDATE readposts SET lastpostread = " . sqlesc($postid) . " WHERE id = " . sqlesc($arr['r_id'])) or sqlerr(__FILE__, __LINE__);
    }
...
function forum_stats()
{
    ...
    if (file_exists($file) && filemtime($file) > (time() - $expire)) {
        ...
        $dt = sqlesc(time() - 180);
        $forum1 = sql_query("SELECT id, username, class, warned, donor, king, pirate, anonymous FROM users WHERE forum_access >= " . sqlesc($dt) . " ORDER BY class DESC") or sqlerr(__FILE__, __LINE__);
        while ($forum2 = mysqli_fetch_assoc($forum1)) {
            $forum3[] = $forum2;
        }
    ...
    $topic_post_arr = mysqli_fetch_assoc($topic_post_res);
...
function show_forums($forid, $subforums = false, $sfa = "", $mods_array = "", $show_mods = false)
{
    ...
    while ($forums_arr = mysqli_fetch_assoc($forums_res)) {
        ...
        $forumid    = intval($forums_arr["id"]);
        $lastpostid = intval($forums_arr['lastpost']);
        ...
        if ($subforums == false && !empty($sfa[$forumid])) {
            ...
            $topics = intval($forums_arr["topiccount"] + $subtopics);
            $posts  = intval($forums_arr["postcount"] + $subposts);
        } else {
            $topics = intval($forums_arr["topiccount"]);
            $posts  = intval($forums_arr["postcount"]);
        }
...
function update_topic_last_post($topicid)
{
    ...
    sql_query("UPDATE topics SET lastpost = " . sqlesc($arr['id']) . " WHERE id = " . sqlesc($topicid)) or sqlerr(__FILE__, __LINE__);
}
...
function insert_compose_frame($id, $newtopic = true, $quote = false, $attachment = false)
{
    ...
    if ($quote) {
        ...
        $res = sql_query("SELECT posts.*, users.username FROM posts JOIN users ON posts.userid = users.id WHERE posts.id = " . sqlesc($postid)) or sqlerr(__FILE__, __LINE__);
        ...
        if (mysqli_num_rows($res) == 0) {
            ...
            $htmlout .= end_table();
            $htmlout .= end_main_frame();
            echo stdhead("Error - No post with this ID") . $htmlout . stdfoot();
            exit();
        }
    ...
    if (!$newtopic) {
        $postres = sql_query("SELECT p.id, p.added, p.body, p.anonymous, u.id AS uid, u.username, u.avatar, u.offavatar " . "FROM posts AS p " . "LEFT JOIN users AS u ON u.id = p.userid " . "WHERE p.topicid = " . sqlesc($id) . " " . "ORDER BY p.id DESC LIMIT 10") or sqlerr(__FILE__, __LINE__);
            ...
            while ($post = mysqli_fetch_assoc($postres)) {
                ...
                if ($post["anonymous"] == "yes")
                    if ($CURUSER['class'] < UC_MODERATOR && $post["uid"] != $CURUSER["id"]) {
                        ...
                        $htmlout .= "<p class='sub'>#" . intval($post["id"]) . " by <i>Anonymous</i> (<b>" . htmlspecialchars($post["username"]) . "</b>) at " . get_date($post["added"], 'LONG', 1, 0) . "</p>";
                    } else
                    $htmlout .= "<p class='sub'>#" . intval($post["id"]) . " by " . (!empty($post["username"]) ? htmlspecialchars($post["username"]) : "unknown[" . intval($post['uid']) . "]") . " at " . get_date($post["added"], 'LONG', 1, 0) . "</p>";
...
if ($action == 'updatetopic') {
    ...
    if (isMod($topic_arr["forumid"]) || $CURUSER['class'] >= UC_MODERATOR) {
        if (($CURUSER['class'] < $topic_arr['minclasswrite']) && !isMod($topic_arr["forumid"]))
            stderr('Error...', 'You are not allowed to edit this topic.');
        
        $forumid = (int) $topic_arr['forumid'];
        $subject = htmlspecialchars($topic_arr['subject']);
        
        if ((isset($_GET['delete']) ? htmlspecilchars($_GET['delete']) : (isset($_POST['delete']) ? htmlspecialchars($_POST['delete']) : '')) == 'yes') {
        ...
        $new_subject = htmlspecialchars($_POST['subject']);
    ...
    if ($CURUSER['class'] == MAX_CLASS || isMod($forumid)) {
        ...
        $res = sql_query("SELECT name, description, minclassread, minclasswrite, minclasscreate FROM forums WHERE id = " . sqlesc($forumid)) or sqlerr(__FILE__, __LINE__);
        if (mysqli_num_rows($res) == 0)
        ...
        $HTMLOUT .= end_frame();
        $HTMLOUT .= end_main_frame();
        echo stdhead("{$lang['forums_title']}") . $HTMLOUT . stdfoot();
        exit();
    }
    ...
    if ($CURUSER['class'] == MAX_CLASS || isMod($forumid)) {
        ...
        $res = sql_query('SELECT id FROM forums WHERE id = ' . sqlesc($forumid)) or sqlerr(__FILE__, __LINE__);
        if (mysqli_num_rows($res) == 0)
    ...
    if ($CURUSER['class'] == MAX_CLASS || isMod($forumid)) {
        ...
        $confirmed = isset($_GET['confirmed']) && (int) $_GET['confirmed'];
        if (!$confirmed) {
            ...
            if ($topics > 0) {
                ...
                $rp = sql_query("SELECT COUNT(id) FROM posts WHERE topicid IN (" . join(', ', $ids) . ")") or sqlerr(__FILE__, __LINE__);
                foreach ($ids as $id)
    ...
    $HTMLOUT .= end_main_frame();
    echo stdhead("New Topic") . $HTMLOUT . stdfoot();
    exit();
    ...
    if ($use_flood_mod && $CURUSER['class'] < UC_MODERATOR && !isMod($forumid)) {
        $res = sql_query("SELECT COUNT(id) AS c FROM posts WHERE userid = " . sqlesc($CURUSER['id']) . " AND added > '" . (time() - ($minutes * 60)) . "'") or sqlerr(__FILE__, __LINE__);
        $arr = mysqli_fetch_assoc($res);
    ...
    if ($newtopic) {
        ...
        sql_query("INSERT INTO topics (userid, forumid, subject, anonymous) VALUES(" . sqlesc($userid) . ", " . sqlesc($forumid) . ", $subject, " . sqlesc($anonymous) . ")") or sqlerr(__FILE__, __LINE__);
        ...
        $res = sql_query("SELECT locked, subject FROM topics WHERE id = " . sqlesc($topicid)) or sqlerr(__FILE__, __LINE__);
        if (mysqli_num_rows($res) == 0)
        ...
        $res_sub = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT userid FROM subscriptions  WHERE topicid = " . sqlesc($topicid)) or sqlerr(__FILE__, __LINE__);
        ...
        // ===end
        //------ Check double post     
        $doublepost = sql_query("SELECT p.id, p.added, p.userid, p.body, t.lastpost, t.id " . "FROM posts AS p " . "INNER JOIN topics AS t ON p.id = t.lastpost " . "WHERE t.id = " . sqlesc($topicid) . " AND p.userid = " . sqlesc($userid) . " AND p.added > " . (time() - 1 * 86400) . " " . "ORDER BY p.added asc LIMIT 1") or sqlerr(__FILE__, __LINE__);
        if (mysqli_num_rows($doublepost) == 0 || $CURUSER['class'] >= UC_MODERATOR) {
            $added     = sqlesc(time());
            $body      = sqlesc($body);
            $anonymous = (isset($_POST['anonymous']) && $_POST["anonymous"] != "" ? "yes" : "no");
            sql_query("INSERT INTO posts (topicid, userid, added, body, anonymous, posticon) VALUES(" . sqlesc($topicid) . ", " . sqlesc($userid) . ", $added, $body, " . sqlesc($anonymous) . ", " . sqlesc($posticon) . ")") or sqlerr(__FILE__, __LINE__);
            ...
            sql_query("UPDATE posts SET body = " . sqlesc(trim($results['body']) . "\n\n" . $body) . ", editedat = " . time() . ", editedby = " . sqlesc($userid) . ", posticon=" . sqlesc($posticon) . " WHERE id=" . sqlesc($postid)) or sqlerr(__FILE__, __LINE__);
        }
    }
    
    if ($use_attachment_mod && ((isset($_POST['uploadattachment']) ? htmlspecialchars($_POST['uploadattachment']) : '') == 'yes')) {
        $file = $_FILES['file'];
        
        $fname   = trim(htmlspecialchars($file['name']));
        $size    = $file['size'];
        ...
        if (empty($uploaderror)) {
            sql_query("INSERT INTO attachments (topicid, postid, filename, size, owner, added, type) VALUES (" . sqlesc($topicid) . "," . sqlesc($postid) . "," . sqlesc($fname) . ", " . sqlesc($size) . ", " . sqlesc($userid) . ", " . sqlesc(time()) . ", " . sqlesc($type) . ")") or sqlerr(__FILE__, __LINE__);
    ...
    // ------ Get topic info
    $res = sql_query("SELECT " . ($use_poll_mod ? 't.pollid, ' : '') . "t.locked, t.subject, t.sticky, t.userid AS t_userid, t.forumid, f.name AS forum_name, f.minclassread, f.minclasswrite, f.minclasscreate, (SELECT COUNT(id)FROM posts WHERE topicid = t.id) AS p_count " . "FROM topics AS t " . "LEFT JOIN forums AS f ON f.id = t.forumid " . "WHERE t.id = " . sqlesc($topicid)) or sqlerr(__FILE__, __LINE__);
    ...
    // ------ Update hits column
    mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE topics SET views = views + 1 WHERE id=" . sqlesc($topicid)) or sqlerr(__FILE__, __LINE__);
    //------ Make page menu
    ...
    if ($page[0] == "p") {
        ...
        $res = sql_query("SELECT id FROM posts WHERE topicid=" . sqlesc($topicid) . " ORDER BY added") or sqlerr(__FILE__, __LINE__);
        $i = 1;
        ...
        if (mysqli_num_rows($res) > 0) {
            ...
            if ($CURUSER['class'] >= UC_ADMINISTRATOR) {
                if (!$listvotes)
                    ...
                    $res4 = sql_query("SELECT pa.userid, u.username, u.anonymous FROM postpollanswers AS pa LEFT JOIN users AS u ON u.id = pa.userid WHERE pa.pollid = " . sqlesc($pollid)) or sqlerr(__FILE__, __LINE__);
                    $voters = '';
...
/*]]>*/
    ...
    while ($arr = mysqli_fetch_assoc($res)) {
        ...
        $posticon          = ($arr["posticon"] > 0 ? "<img src=\"pic/post_icons/icon" . htmlspecialchars($arr["posticon"]) . ".gif\" style=\"padding-left:3px;\" alt=\"post icon\" title=\"post icon\" />" : "&nbsp;");
        ...
        $postername = htmlspecialchars($arr['uusername']);
        $avatar     = ($CURUSER["avatars"] == "all" ? htmlspecialchars($arr["avatar"]) : ($CURUSER["avatars"] == "some" && $arr["offavatar"] == "no" ? htmlspecialchars($arr["avatar"]) : ""));
        $title      = (!empty($postername) ? (empty($arr['title']) ? "(" . get_user_class_name($arr['class']) . ")" : "(" . (htmlspecialchars($arr['title'])) . ")") : '');
        $forumposts = (!empty($postername) ? ($arr['posts_count'] != 0 ? intval($arr['posts_count']) : 'N/A') : 'N/A');
        if ($arr["p_anon"] == "yes") {
    ...
    echo stdhead("Forums :: View Topic: $subject") . $HTMLOUT . stdfoot();
    ...
    $HTMLOUT .= end_main_frame();
    echo stdhead("Post quote") . $HTMLOUT . stdfoot();
    exit();
    ...
    $HTMLOUT .= end_main_frame();
    echo stdhead("Post reply") . $HTMLOUT . stdfoot();
    exit();
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (!isset($_POST['lasteditedby']))
            sql_query("UPDATE posts SET body = " . sqlesc($body) . ", editedat = " . sqlesc(time()) . ", editedby = " . sqlesc($CURUSER['id']) . ", posticon = " . sqlesc($posticon) . " WHERE id = " . sqlesc($postid)) or sqlerr(__FILE__, __LINE__);
        else
            sql_query("UPDATE posts SET body = " . sqlesc($body) . ", posticon = " . sqlesc($posticon) . " WHERE id = " . sqlesc($postid)) or sqlerr(__FILE__, __LINE__);
        
        header("Location: {$_SERVER['PHP_SELF']}?action=viewtopic&topicid=" . intval($arr['topicid']) . "&page=p$postid#p$postid");
        exit();
    }
    ...
    $HTMLOUT .= end_main_frame();
    echo stdhead("Edit Post") . $HTMLOUT . stdfoot();
    exit();
    ...
    $r = sql_query("SELECT t.id,t.subject " . ($use_poll_mod ? ",t.pollid" : "") . ",t.forumid,(SELECT COUNT(p.id) FROM posts as p where p.topicid=" . $topicid . ") AS posts FROM topics as t WHERE t.id=" . sqlesc($topicid)) or sqlerr(__FILE__, __LINE__);
    ...
    if ($CURUSER["class"] >= UC_MODERATOR || isMod($a["forumid"])) {
        ...
        if (!$sure)
            ...
            header('Location: ' . $_SERVER['PHP_SELF'] . '?action=viewforum&forumid=' . intval($a["forumid"]));
            exit();
        }
    }
    ...
    $res = sql_query("SELECT p.topicid " . ($use_attachment_mod ? ", a.filename" : "") . ", t.forumid, (SELECT COUNT(id) FROM posts WHERE topicid=p.topicid) AS posts_count, " . "(SELECT MAX(id) FROM posts WHERE topicid=p.topicid AND id < p.id) AS p_id " . "FROM posts AS p " . "LEFT JOIN topics as t on t.id=p.topicid " . ($use_attachment_mod ? "LEFT JOIN attachments AS a ON a.postid = p.id " : "") . "WHERE p.id=" . sqlesc($postid)) or sqlerr(__FILE__, __LINE__);
    ...
    if (isMod($arr["forumid"]) || $CURUSER['class'] >= UC_MODERATOR) {
        ...
        $redirtopost = (is_valid_id($arr['p_id']) ? "&page=p" . intval($arr['p_id']) . "#p" . intval($arr['p_id']) : '');
    ...
    $sure = isset($_GET['sure']) && (int) $_GET['sure'];
    if (!$sure || $sure != 1)
        stderr('Sanity check...', 'You are about to delete a poll. Click <a href=' . $_SERVER['PHP_SELF'] . '?action=' . htmlspecialchars($action) . '&amp;pollid=' . intval($arr['id']) . '&amp;sure=1>here</a> if you are sure.');
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && !$topicid) {
        $topicid = ($subaction == "edit" ? intval($poll['tid']) : intval($_POST["updatetopicid"]));
        ...
        if ($subaction == "edit" && is_valid_id($pollid))
            ...
            sql_query("INSERT INTO postpolls VALUES(id" . ", " . sqlesc(time()) . ", " . sqlesc($question) . ", " . sqlesc($option0) . ", " . sqlesc($option1) . ", " . sqlesc($option2) . ", " . sqlesc($option3) . ", " . sqlesc($option4) . ", " . sqlesc($option5) . ", " . sqlesc($option6) . ", " . sqlesc($option7) . ", " . sqlesc($option8) . ", " . sqlesc($option9) . ", " . sqlesc($option10) . ", " . sqlesc($option11) . ", " . sqlesc($option12) . ", " . sqlesc($option13) . ", " . sqlesc($option14) . ", " . sqlesc($option15) . ", " . sqlesc($option16) . ", " . sqlesc($option17) . ", " . sqlesc($option18) . ", " . sqlesc($option19) . ", " . sqlesc($sort) . ")") or sqlerr(__FILE__, __LINE__);
  ...
  <input type='hidden' name='action' value='" . $action . "' />
    ...
    $HTMLOUT .= end_main_frame();
    echo stdhead("Polls") . $HTMLOUT . stdfoot();
    ...
    if (mysqli_num_rows($res) == 0)
        sql_query("INSERT INTO attachmentdownloads (fileid, username, userid, date, downloads) VALUES (" . sqlesc($id) . ", " . sqlesc($CURUSER['username']) . ", " . sqlesc($CURUSER['id']) . ", " . time() . ", 1)") or sqlerr(__FILE__, __LINE__);
    else
    ...
    header("Content-Type: " . htmlspecialchars($arr['type']));
    ...
    $res = sql_query("SELECT fileid, at.filename, userid, username, atdl.downloads, date, at.downloads AS dl " . "FROM attachmentdownloads AS atdl " . "LEFT JOIN attachments AS at ON at.id=atdl.fileid " . "WHERE fileid = " . sqlesc($fileid) . ($CURUSER['class'] < UC_MODERATOR ? " AND owner=" . sqlesc($CURUSER['id']) : '')) or sqlerr(__FILE__, __LINE__);
  ...
  <body>
	...
	<td>Date</td>
        ...
        $dls     = 0;
        while ($arr = mysqli_fetch_assoc($res)) {
            $HTMLOUT .= "<tr align='center'>" . "<td>" . htmlspecialchars($arr['filename']) . "</td>" . "<td><a class='pointer' onclick=\"opener.location=('/userdetails.php?id=" . (int) $arr['userid'] . "'); self.close();\">" . htmlspecialchars($arr['username']) . "</a></td>" . "<td>" . (int) $arr['downloads'] . "</td>" . "<td>" . get_date($arr['date'], 'DATE', 1, 0) . " (" . get_date($arr['date'], 'DATE', 1, 0) . ")</td>" . "</tr>";
    ...
    if ((isset($_POST[$action]) ? htmlspecialchars($_POST[$action]) : '') == 'clear') {
        ...
        if ($count > 0) {
            ...
            $HTMLOUT .= end_main_frame();
            echo stdhead("Catch Up") . $HTMLOUT . stdfoot();
            die();
        } else
    ...
    $res = sql_query('SELECT COUNT(p.id) AS post_count ' . 'FROM posts AS p ' . 'LEFT JOIN topics AS t ON t.id = p.topicid ' . 'LEFT JOIN forums AS f ON f.id = t.forumid ' . 'WHERE p.added > ' . time() . ' - 86400 AND f.minclassread <= ' . sqlesc($CURUSER['class'])) or sqlerr(__FILE__, __LINE__);
    ...
    $res = sql_query('SELECT p.id AS pid, p.topicid, p.userid AS userpost, p.added, t.id AS tid, t.subject, t.forumid, t.lastpost, t.views, f.name, f.minclassread, f.topiccount, u.username ' . 'FROM posts AS p ' . 'LEFT JOIN topics AS t ON t.id = p.topicid ' . 'LEFT JOIN forums AS f ON f.id = t.forumid ' . 'LEFT JOIN users AS u ON u.id = p.userid ' . 'LEFT JOIN users AS topicposter ON topicposter.id = t.userid ' . 'WHERE p.added > ' . time() . ' - 86400 AND f.minclassread <= ' . sqlesc($CURUSER['class']) . ' ' . 'ORDER BY p.added DESC ' . $pager["limit"]) or sqlerr(__FILE__, __LINE__);
    ...
    while ($getdaily = mysqli_fetch_assoc($res)) {
        ...
        $HTMLOUT .= "<tr>
			<td align='left'>
		  <a href='" . $_SERVER['PHP_SELF'] . "?action=viewtopic&amp;topicid=" . intval($getdaily['tid']) . "&amp;page=" . $postid . "#" . $postid . "'>" . htmlspecialchars($getdaily['subject']) . "</a><br />
    ...
    $HTMLOUT .= $pager['pagerbottom'];
    $HTMLOUT .= end_main_frame();
    echo stdhead('Today Posts (Last 24 Hours)') . $HTMLOUT . stdfoot();
} else if ($action == "search") //-------- Action: Search
    {
    $error    = false;
    $found    = '';
    $keywords = (isset($_GET['keywords']) ? trim(htmlspecialchars($_GET['keywords'])) : '');
    if (!empty($keywords)) {
        ...
        if ($count == 0)
            ...
            $res     = sql_query("SELECT p.id, p.topicid, p.userid, p.added, t.forumid, t.subject, f.name, f.minclassread, u.username " . "FROM posts AS p " . "LEFT JOIN topics AS t ON t.id=p.topicid " . "LEFT JOIN forums AS f ON f.id=t.forumid " . "LEFT JOIN users AS u ON u.id=p.userid " . "WHERE p.body LIKE " . sqlesc("%" . $keywords . "%") . " " . $pager['limit'] . "");
  ...
  <font class='small'><b>Note:</b> Searches <u>only</u> in posts.</font></td>
    ...
    echo stdhead("Forum Search") . $HTMLOUT . stdfoot();
    exit();
    ...
    $res = sql_query("SELECT name FROM overforums WHERE id = " . sqlesc($ovfid)) or sqlerr(__FILE__, __LINE__);
    ...
    $HTMLOUT .= end_main_frame();
    echo stdhead("Forums") . $HTMLOUT . stdfoot();
    exit();
    ...
    $sub_forums = sql_query(" SELECT f.id, f2.name, f2.id AS subid,f2.postcount,f2.topiccount, p.added, p.anonymous, p.userid, p.id AS pid, u.username, t.subject,t.id as tid,r.lastpostread,t.lastpost FROM forums AS f LEFT JOIN forums AS f2 ON f2.place = f.id AND f2.minclassread<" . sqlesc($CURUSER["class"]) . " LEFT JOIN posts AS p ON p.id = (SELECT MAX(lastpost) FROM topics WHERE forumid = f2.id ) LEFT JOIN users AS u ON u.id = p.userid LEFT JOIN topics AS t ON t.id = p.topicid LEFT JOIN readposts AS r ON r.userid =" . sqlesc($CURUSER["id"]) . " AND r.topicid = p.topicid ORDER BY t.lastpost ASC, f2.name , f.id ASC") or sqlerr(__FILE__, __LINE__);
    while ($a = mysqli_fetch_assoc($sub_forums)) {
        if ($a["subid"] == 0)
            $forums[$a["id"]] = false;
        else {
            $forums[$a["id"]]["lastpost"] = array(
            ...
            $forums[$a["id"]]["count"][]  = array(
                "posts" => $a["postcount"],
                "topics" => $a["topiccount"]
            );
            $forums[$a["id"]]["topics"][] = array(
                "id" => $a["subid"],
                "name" => $a["name"],
                "new" => ($a["lastpost"]) != $a["lastpostread"] ? 1 : 0
            );
        }
    }
    $r_mod = sql_query("SELECT f.id,m.user,m.uid FROM forums as f LEFT JOIN forum_mods as m ON f.id = m.fid ORDER BY f.id") or sqlerr(__FILE__, __LINE__);
    
    while ($a_mod = mysqli_fetch_assoc($r_mod)) {
        if (!isset($a_mod["uid"]))
            $f[$a_mod["id"]] = false;
        else
            $f_mod[$a_mod["id"]][] = array(
                "user" => $a_mod["user"],
                "id" => $a_mod["uid"]
            );
    }
    ...
    while ($ovf_arr = mysqli_fetch_assoc($ovf_res)) {
      ...
      <b><font color='white'>" . $ovfname . "</font></b></a></td>
    ...
    $HTMLOUT .= "</p>";
    $HTMLOUT .= end_main_frame();
    echo stdhead("Forum") . $HTMLOUT . stdfoot();
}
?>
