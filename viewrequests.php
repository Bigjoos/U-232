...
<a class='altlink' href='viewrequests.php?requestorid=" . intval($CURUSER['id']) . "'>View my requests</a></p>
";

$search = (isset($_GET['search']) ? ' AND requests.request like ' . sqlesc('%' . $_GET['search'] . '%') . ' ' : '');

if ($requestorid != 0)
    $category = ($categ != 0 ? 'WHERE requests.cat = ' . sqlesc($categ) . ' AND requests.userid = ' . sqlesc($requestorid) : 'WHERE requests.userid = ' . sqlesc($requestorid));
else
    $category = ($categ != 0 ? 'WHERE requests.cat = ' . sqlesc($categ) : 'WHERE requests.cat != ""');
...
" . ($CURUSER['class'] >= UC_MODERATOR ? " | <a class='altlink' href='viewrequests.php?category=" . $categ . "&amp;sort=" . $sort['link'] . "&amp;filter=false'>Only Filled</a>" : '') . '
</form>
';
...
';
    ...
    while ($arr = mysqli_fetch_assoc($res)) {
        ...
        $u1['id']        = intval($arr['userid']);
        $u1['username']  = intval($arr['requester']);
        $u1['class']     = intval($arr['u1class']);
        $u1['enabled']   = htmlspecialchars($arr['u1enabled']);
        $u1['donor']     = htmlspecialchars($arr['u1donor']);
        $u1['warned']    = htmlspecialchars($arr['u1warned']);
        $u1['leechwarn'] = htmlspecialchars($arr['u1leechwarn']);
        $u1['chatpost']  = htmlspecialchars($arr['u1chatpost']);
        $u1['pirate']    = htmlspecialchars($arr['u1pirate']);
        $u1['king']      = htmlspecialchars($arr['u1king']);
        $u2['id']        = htmlspecialchars($arr['filledby']);
        $u2['username']  = htmlspecialchars($arr['filler']);
        $u2['class']     = intval($arr['u2class']);
        $u2['enabled']   = htmlspecialchars($arr['u2enabled']);
        $u2['donor']     = htmlspecialchars($arr['u2donor']);
        $u2['warned']    = htmlspecialchars($arr['u2warned']);
        $u2['leechwarn'] = htmlspecialchars($arr['u2leechwarn']);
        $u2['chatpost']  = htmlspecialchars($arr['u2chatpost']);
        $u2['pirate']    = htmlspecialchars($arr['u2pirate']);
        $u2['king']      = htmlspecialchars($arr['u2king']);
        ...
        $filled   = ($arr['torrentid'] != 0 ? '<a href="details.php?id=' . intval($arr['torrentid']) . '"><span style="color:green;"><b>Yes</b></span></a>' : '<a href="viewrequests.php?id=' . intval($arr['id']) . '&amp;req_details"><span style="color:red;"><b>No</b></span></a>');
...
<td align="left"><a href="viewrequests.php?id=' . intval($arr['id']) . '&amp;req_details"><b>' . htmlspecialchars($arr['request']) . '</b></a></td>
<td align="center">' . get_date($arr['added'], '') . '</td>
<td align="center">' . $addedby . '</td>
<td align="center">' . $filled . '</td>
<td align="center"><strong>' . $filledby . '</strong></td>
<td align="center"><a href="viewrequests.php?id=' . intval($arr['id']) . '&amp;votes_view"><b>' . intval($arr['hits']) . '</b></a></td>
';
        
        if ($CURUSER['class'] >= UC_MODERATOR)
            $HTMLOUT .= '<td><input type="checkbox" name="delreq[]" value="' . intval($arr['id']) . '" /></td>';
...
/////////////////////// HTML OUTPUT //////////////////////////////
echo stdhead('View Requests') . $HTMLOUT . stdfoot();
?>
