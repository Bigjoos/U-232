...
function stdhead($title = "", $msgalert = true, $stdhead = false) {
    ...
    <div class='itunes'>
	  ...
	  <div class='play1'></div>
	  <div class='play3'></div>
	  <div class='play2'>{$INSTALLER09['site_name']}</div>
	  </div>
	  </td></tr>";
	  $htmlout.=StatusBar();
...
function StatusBar() {
  ...
  $res1 = sql_query("SELECT count(id) FROM messages WHERE receiver=" . sqlesc($CURUSER["id"]) . " AND unread='yes'") or sqlerr(__LINE__,__FILE__);
	$arr1 = mysqli_fetch_row($res1);
	$unread = $arr1[0];
	$inbox = ($unread == 1 ? "$unread&nbsp;{$lang['gl_msg_singular']}" : "$unread&nbsp;{$lang['gl_msg_plural']}");
	$res2 = sql_query("SELECT seeder, count(*) AS pCount FROM peers WHERE userid=".sqlesc($CURUSER['id'])." GROUP BY seeder") or sqlerr(__LINE__,__FILE__);
   ...
   ////////////// REP SYSTEM END //////////
    ...
    $usrclass = "&nbsp;<a href='{$INSTALLER09['baseurl']}/setclass.php'><b>(".get_user_class_name($CURUSER['class']).")</b></a>&nbsp;";
	  $StatusBar = '';
		...
		&nbsp;Bonus:&nbsp;<a href='{$INSTALLER09['baseurl']}/mybonus.php'>{$CURUSER['seedbonus']}</a>&nbsp;|&nbsp;<a href='logout.php'>[{$lang['gl_logout']}]</a>";
		if(!$INSTALLER09['coins'])
		$StatusBar .= "
