...
function stdhead($title = "", $msgalert = true, $stdhead = false) {
   ...
   <tr>
     ...
     <img src="'.$INSTALLER09['pic_base_url'].'cat_free.gif" alt="FREE!" /></td>
...
global $CURUSER, $INSTALLER09, $q, $queries, $query_stat, $querytime;
...
function StatusBar() {
	global $CURUSER, $INSTALLER09, $lang, $rep_is_on, $mc, $msgalert;
	if (!$CURUSER)
  ...
  $res1 = @sql_query("SELECT count(id) FROM messages WHERE receiver=" . $CURUSER["id"] . " AND unread='yes'") or sqlerr(__LINE__,__FILE__);
    ...
    $usrclass = "&nbsp;<a href='./setclass.php'><b>(".get_user_class_name($CURUSER['class']).")</b></a>";
	  ...
	  $StatusBar .= "<br />{$lang['gl_ratio']}:$ratio".
		...
		if($INSTALLER09['coins'])$StatusBar .= "&nbsp;&nbsp;{$lang['gl_coins']}:<a href='./coins.php'>{$CURUSER['coins']}&nbsp;</a>|&nbsp;";
		...
		&nbsp;{$lang['gl_act_torrents']}:&nbsp;<img alt='{$lang['gl_seed_torrents']}' title='{$lang['gl_seed_torrents']}' src='{$INSTALLER09['pic_base_url']}up.png' />&nbsp;{$seedleech['yes']}".
